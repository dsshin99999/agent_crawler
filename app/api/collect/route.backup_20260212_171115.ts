...
async function fetchOfficialSiteCandidates(keyword: string) {
  ...
  const query = `${keyword} 공식몰 온라인 제품 스토어 official website official store online product store`;
  const url =
...
async function verifyOfficialSiteWithGemini(
  ...
  const prompt = [
    "당신은 브랜드/제품의 공식 출처를 판별하는 검증기입니다.",
    `브랜드: ${brand}`,
    `제품명: ${productName}`,
    "",
    "후보 URL 목록(SerpAPI 결과)만을 근거로 판단하세요.",
    "아래 항목을 JSON으로 반환하세요:",
    "- officialHomepage: 브랜드/제조사 공식 홈페이지",
    ...
    "- officialKo: 공식 한글명",
    "- productKeywords 규칙:",
    "- product_keyword1(original): 입력 제품명(productName) 문자열에서 브랜드명을 제외하고, 제품 유형을 가장 잘 설명하는 핵심 단어 1개를 원문에서 뽑는다. 같은 의미의 다른 단어로 변형 금지",
    "- product_keyword2(ko): product_keyword1이 한글이면 원문 그대로 복사하고, 영문이면 한글 단어로 번역해서 저장한다.",
    "- product_keyword3(en): product_keyword1이 영문이면 원문 그대로 복사하고, 한글이면 영문 단어로 번역해서 저장한다.",
